...
const index_js_1 = require("../../../plugins/auth0/src/index.js");
const use_masked_errors_js_1 = require("../../src/plugins/use-masked-errors.js");
describe('useMaskedErrors', () => {
    ...
    `,
        resolvers: {
            Query: {
                ...
                secretEnvelop: () => {
                    throw (0, utils_1.createGraphQLError)('This message goes to all the clients out there!', {
                        extensions: { foo: 1 },
                    });
                },
            ...
            Subscription: {
                ...
                instantGraphQLError: {
                    subscribe: async function () {
                        throw (0, utils_1.createGraphQLError)('Noop');
                    },
                    resolve: _ => _,
                },
                streamGraphQLError: {
    ...
    it('Should mask non GraphQLErrors', async () => {
        const testInstance = (0, testing_1.createTestkit)([(0, use_masked_errors_js_1.useMaskedErrors)()], schema);
        const result = await testInstance.execute(`query { secret }`);
        (0, testing_1.assertSingleExecutionValue)(result);
        expect(result.errors).toBeDefined();
        expect(result.errors).toHaveLength(1);
        const [error] = result.errors;
        expect(error.message).toEqual(use_masked_errors_js_1.DEFAULT_ERROR_MESSAGE);
    });
    ...
    it('Should forward extensions from GraphQLError to final GraphQLError in errors array', async () => {
        ...
        expect(JSON.stringify(result)).toMatchInlineSnapshot(`"{"errors":[{"message":"This message goes to all the clients out there!","locations":[{"line":1,"column":9}],"path":["secretWithExtensions"],"extensions":{"code":"Foo","message":"Bar"}}],"data":null}"`);
    });
    it('Should properly mask context creation errors with a custom error message', async () => {
        ...
        const testInstance = (0, testing_1.createTestkit)([
            ...
            (0, use_masked_errors_js_1.useMaskedErrors)({ errorMessage: 'My Custom Error Message.' }),
        ], schema);
        try {
    ...
    it('Should properly mask context creation errors', async () => {
        ...
        const testInstance = (0, testing_1.createTestkit)([
            ...
            (0, use_masked_errors_js_1.useMaskedErrors)(),
        ], schema);
        try {
            await testInstance.execute(`query { secretWithExtensions }`);
        }
        catch (err) {
            expect(err.message).toEqual(use_masked_errors_js_1.DEFAULT_ERROR_MESSAGE);
        }
    });
    it('Should not mask expected context creation errors', async () => {
        ...
        const testInstance = (0, testing_1.createTestkit)([
            (0, core_1.useExtendContext)(() => {
                throw (0, utils_1.createGraphQLError)('No context for you!', { extensions: { foo: 1 } });
            }),
            (0, use_masked_errors_js_1.useMaskedErrors)(),
        ], schema);
        try {
    ...
    it('Should mask subscribe (sync/promise) subscription errors with a custom error message', async () => {
        const testInstance = (0, testing_1.createTestkit)([(0, use_masked_errors_js_1.useMaskedErrors)({ errorMessage: 'My Custom subscription error message.' })], schema);
        const result = await testInstance.execute(`subscription { instantError }`);
        (0, testing_1.assertSingleExecutionValue)(result);
        expect(result.errors).toBeDefined();
    ...
    it('Should mask subscribe (AsyncIterable) subscription errors', async () => {
        ...
        try {
            await (0, testing_1.collectAsyncIteratorValues)(resultStream);
        }
        catch (err) {
            expect(err.message).toEqual(use_masked_errors_js_1.DEFAULT_ERROR_MESSAGE);
        }
    ...
    it('Should mask subscribe (AsyncIterable) subscription errors with a custom error message', async () => {
        expect.assertions(1);
        const testInstance = (0, testing_1.createTestkit)([(0, use_masked_errors_js_1.useMaskedErrors)({ errorMessage: 'My AsyncIterable Custom Error Message.' })], schema);
        const resultStream = await testInstance.execute(`subscription { streamError }`);
        (0, testing_1.assertStreamExecutionValue)(resultStream);
        try {
            await (0, testing_1.collectAsyncIteratorValues)(resultStream);
    ...
    it('Should mask resolve subscription errors with a custom error message', async () => {
        const testInstance = (0, testing_1.createTestkit)([(0, use_masked_errors_js_1.useMaskedErrors)({ errorMessage: 'Custom resolve subscription errors.' })], schema);
        const resultStream = await testInstance.execute(`subscription { streamResolveError }`);
        (0, testing_1.assertStreamExecutionValue)(resultStream);
        const allResults = await (0, testing_1.collectAsyncIteratorValues)(resultStream);
        expect(allResults).toHaveLength(1);
        const [result] = allResults;
        expect(JSON.stringify(result)).toMatchInlineSnapshot(`"{"errors":[{"message":"Custom resolve subscription errors."}],"data":{"streamResolveError":null}}"`);
    });
    ...
    it('Should not mask auth0 header errors', async () => {
        ...
        const testInstance = (0, testing_1.createTestkit)([(0, use_masked_errors_js_1.useMaskedErrors)(), (0, index_js_1.useAuth0)(auto0Options)], schema);
        try {
    ...
    it('should use custom error mask function for execution errors', async () => {
        const customErrorMaskFn = e => new graphql_1.GraphQLError('Custom error message for ' + e, null, null, null, null, null, {
            custom: true,
        });
        const testInstance = (0, testing_1.createTestkit)([(0, use_masked_errors_js_1.useMaskedErrors)({ maskError: customErrorMaskFn })], schema);
        const result = await testInstance.execute(`query { secret }`);
        (0, testing_1.assertSingleExecutionValue)(result);
        expect(result).toMatchInlineSnapshot(`
      {
    ...
    `);
        expect(JSON.stringify(result)).toMatchInlineSnapshot(`"{"errors":[{"message":"Custom error message for Secret sauce that should not leak.\\n\\nGraphQL request:1:9\\n1 | query { secret }\\n  |         ^","extensions":{"custom":true}}],"data":null}"`);
    });
    ...
    it('should use custom error mask function for errors while building the context', async () => {
        ...
        const testInstance = (0, testing_1.createTestkit)([
            (0, use_masked_errors_js_1.useMaskedErrors)({ maskError: customErrorMaskFn }),
            (0, core_1.useExtendContext)(() => {
                throw (0, utils_1.createGraphQLError)('Custom error');
                return {};
            }),
        ], schema);
        try {
            ...
            expect(e.message).toEqual('Custom error message for Custom error');
        }
        expect.assertions(1);
    });
    it('should include the original error message stack in the extensions in development mode', async () => {
      ...
      `,
        ...
        const testInstance = (0, testing_1.createTestkit)([(0, use_masked_errors_js_1.useMaskedErrors)({ maskError: (0, use_masked_errors_js_1.createDefaultMaskError)(true) })], schema);
