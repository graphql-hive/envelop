...
describe('oneOf', () => {
    const astSchema = (0, graphql_1.buildSchema)(/* GraphQL */ `
    ${index_js_1.ONE_OF_DIRECTIVE_SDL}
  ...
  `);
    ...
    const GraphQLUserUniqueCondition = new graphql_1.GraphQLInputObjectType({
    ...
    const GraphQLNestedOneOfFieldInput = new graphql_1.GraphQLInputObjectType({
        name: 'NestedOneOfFieldInput',
        fields: {
            field: {
                type: new graphql_1.GraphQLNonNull(GraphQLUserUniqueCondition),
            },
        },
    });
    const GraphQLDeeplyNestedOneOfFieldInput = new graphql_1.GraphQLInputObjectType({
        name: 'DeeplyNestedOneOfFieldInput',
        fields: {
            field: {
                type: new graphql_1.GraphQLNonNull(GraphQLNestedOneOfFieldInput),
            },
        },
    });
    const GraphQLListOneOfInput = new graphql_1.GraphQLInputObjectType({
        name: 'ListOneOfInput',
        fields: {
            items: {
                type: new graphql_1.GraphQLList(new graphql_1.GraphQLNonNull(GraphQLUserUniqueCondition)),
            },
        },
    });
    const GraphQLMutation = new graphql_1.GraphQLObjectType({
        name: 'Mutation',
        fields: {
            test: {
                type: graphql_1.GraphQLString,
    ...
    const GraphQLQuery = new graphql_1.GraphQLObjectType({
        name: 'Query',
        fields: {
            user: {
                type: GraphQLUser,
            ...
            listNonNullOneOf: {
                ...
                args: {
                    input: {
                        type: new graphql_1.GraphQLNonNull(new graphql_1.GraphQLList(new graphql_1.GraphQLNonNull(GraphQLUserUniqueCondition))),
                    },
                },
            },
            inputFieldOneOfList: {
    ...
    const programmaticSchema = new graphql_1.GraphQLSchema({ query: GraphQLQuery, mutation: GraphQLMutation });
    describe.each([
            ...
            it.each([
                ...
                const testInstance = (0, testing_1.createTestkit)([
                    (0, index_js_1.useExtendedValidation)({
                        rules: [index_js_1.OneOfInputObjectsRule],
                    }),
                ], testSchema);
