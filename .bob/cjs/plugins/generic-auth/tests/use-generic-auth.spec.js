...
describe('useGenericAuth', () => {
    ...
    const validresolveUserFn = async (context) => {
        return {
            id: 1,
            name: 'Dotan',
        };
    };
    const invalidresolveUserFn = async (context) => {
        return null;
    };
    describe('protect-all', () => {
        const schemaWithDirective = (0, schema_1.makeExecutableSchema)({
            typeDefs: [
                index_js_1.SKIP_AUTH_DIRECTIVE_SDL,
                /* GraphQL */ `
          type Query {
            test: String!
        ...
        it('handles introspection query operations', async () => {
            const testInstance = (0, testing_1.createTestkit)([
                (0, index_js_1.useGenericAuth)({
                    mode: 'protect-all',
                    resolveUserFn: validresolveUserFn,
                }),
            ], schemaWithDirective);
            const result = await testInstance.execute((0, graphql_1.getIntrospectionQuery)());
        ...
        it('Should inject currentUser into the context when the user is valid', async () => {
            ...
            (0, testing_1.assertSingleExecutionValue)(result);
            expect(result.errors).toBeUndefined();
            expect(spyFn).toHaveBeenCalledWith(expect.objectContaining({
    ...
    describe('auth-directive', () => {
        ...
        it('Should prevent inline fragment of protected type execution when user is not authenticated correctly but continue execution for public fields inside the parent', async () => {
            ...
            const testInstance = (0, testing_1.createTestkit)([
                ...
                {
                    onExecute({ args }) {
                        redactedQuery = (0, graphql_1.print)(args.document).trim();
                    },
                },
            ], schemaWithDirective);
      ...
      `);
        ...
        describe('auth directive with role', () => {
            const validateUserFn = params => {
                const schemaCoordinate = `${params.parentType.name}.${params.fieldNode.name.value}`;
                if (!params.user) {
                    return (0, utils_1.createGraphQLError)(`Accessing '${schemaCoordinate}' requires authentication.`, {
                        nodes: [params.fieldNode],
                    });
                }
                if (params.fieldAuthArgs) {
                    if (params.fieldAuthArgs.role !== params.user.role) {
                        return (0, utils_1.createGraphQLError)(`Missing permissions for accessing field '${schemaCoordinate}'. Requires role '${params.fieldAuthArgs.role}'. Request is authenticated with role '${params.user.role}'.`, { nodes: [params.fieldNode] });
                    }
                }
                return undefined;
            };
            const invalidRoleResolveUserFn = async (context) => {
                return {
                    id: 1,
                    name: 'Dotan',
                    role: 'USER',
            ...
            const validRoleResolveUserFn = async (context) => {
                return {
                    id: 1,
                    name: 'Dotan',
                    role: 'ADMIN',
            ...
            const schemaWithDirectiveWithRole = (0, schema_1.makeExecutableSchema)({
                typeDefs: /* GraphQL */ `
          enum Role {
            ADMIN
            USER
        ...
        `,
            ...
            it('Should prevent field execution when user does not have right role', async () => {
                ...
                (0, testing_1.assertSingleExecutionValue)(result);
                expect(result.errors?.length).toBe(1);
                expect(result.errors?.[0].message).toBe(`Missing permissions for accessing field 'Query.admin'. Requires role 'ADMIN'. Request is authenticated with role 'USER'.`);
            });
