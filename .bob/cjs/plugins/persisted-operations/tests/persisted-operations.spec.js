...
describe('usePersistedOperations', () => {
    const testSchema = (0, schema_1.makeExecutableSchema)({
        resolvers: {
            Query: {
                foo: () => 'test',
            },
    ...
    it('Should allow running persisted operations from source', async () => {
        ...
        const testInstance = (0, testing_1.createTestkit)([
            (0, index_js_1.usePersistedOperations)({
                onlyPersisted: true,
                store,
            }),
        ], testSchema);
    ...
    it('Should fail when persisted operation is not available and `onlyPersisted` is true', async () => {
        const store = new Map([['persisted_1', (0, graphql_1.parse)(`query { foo }`)]]);
        const testInstance = (0, testing_1.createTestkit)([
            (0, index_js_1.usePersistedOperations)({
                onlyPersisted: true,
                store,
    ...
    it('Should allow store function to return actual persisted operations store, using context value', async () => {
        ...
        const customStore = new Map([['persisted_1', (0, graphql_1.parse)(`query { foo }`)]]);
    ...
    it('Should allow setting imperative operation Id, using context value, when source is invalid', async () => {
        ...
        const testInstance = (0, testing_1.createTestkit)([
            (0, index_js_1.usePersistedOperations)({
                onlyPersisted: true,
                store,
                extractOperationId: (context) => context.request.body.operationId,
            }),
        ], testSchema);
