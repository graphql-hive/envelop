...
const print = (ast) => (0, graphql_1.print)(ast).replace(/^\s+|\s+$/g, '');
const allMetrics = {
...
describe('Prom Metrics plugin', () => {
    ...
    function prepare(config, registry = new prom_client_1.Registry(), plugins = []) {
        const plugin = (0, index_js_1.usePrometheus)({
            ...config,
            registry,
        });
        const teskit = (0, testing_1.createTestkit)([
            plugin,
            (0, core_1.useExtendContext)(() => new Promise(resolve => setTimeout(resolve, 250))),
            ...plugins,
        ], schema);
        return {
            ...
            plugin,
            registry,
            async metricString(name) {
                return registry.getSingleMetricAsString(name);
            },
            async allMetrics() {
                return await registry.metrics();
            },
            async metricCount(name, sub = null) {
                const arr = await registry.getMetricsAsJSON();
                const m = arr.find(m => m.name === name);
                if (m) {
                    return (m.values || []).filter((v) => sub === null ? true : v.metricName === `${name}_${sub}`).length;
                }
                return 0;
            },
            async metricValue(name, sub = null) {
                const arr = await registry.getMetricsAsJSON();
                const m = arr.find(m => m.name === name);
                if (m) {
                    return (m.values || []).find((v) => sub === null ? true : v.metricName === `${name}_${sub}`).value;
                }
                return 0;
            },
        };
    }
    function testHistogram(metricName, phases) {
        const histogramFactory = (config) => 
        //@ts-ignore
        (0, index_js_1.createHistogram)({
            ...config,
            histogram: {
                name: metricName,
                help: 'test',
        ...
        it.each(metricDisabledTestCases(metricName, histogramFactory))('should not monitor parse timing when $name', async ({ config }) => {
        ...
        it('should allow to use a custom name within a custom config', async () => {
            ...
            const { execute, metricCount } = prepare({
                metrics: {
                    [metricName]: (0, index_js_1.createHistogram)({
                        registry,
                        histogram: {
                            name: 'metric_test',
                            help: 'test',
    ...
    function testSummary(metricName, phases) {
        const summaryFactory = (config) => 
        //@ts-ignore
        ...
        it.each(metricEnabledTestCases(metricName, phases, summaryFactory))(`should monitor timing when $name`, async ({ config }) => {
            const { execute, metricCount, metricString } = prepare(config, config.registry);
            const result = await execute('query { regularField }');
            (0, testing_1.assertSingleExecutionValue)(result);
            expect(result.errors).toBeUndefined();
        ...
        it.each(metricDisabledTestCases(metricName, summaryFactory))('should not monitor parse timing when $name', async ({ config }) => {
            const { execute, metricCount } = prepare(config, config.registry);
            const result = await execute('query { regularField }');
            (0, testing_1.assertSingleExecutionValue)(result);
            expect(result.errors).toBeUndefined();
        ...
        it('should allow to use a custom name within a custom config', async () => {
            ...
            const { execute, metricCount } = prepare({
                metrics: {
                    [metricName]: (0, utils_js_1.createSummary)({
                        registry,
                        summary: {
                            name: 'metric_test',
                            help: 'test',
    ...
    function testCounter(metricName, phases, query = 'query { regularField }') {
        const counterFactory = (config) => 
        //@ts-ignore
        (0, index_js_1.createCounter)({
            ...config,
            counter: {
                name: metricName,
                help: 'HELP ME',
        ...
        it.each(metricEnabledTestCases(metricName, phases, counterFactory))(`should monitor timing when $name`, async ({ config }) => {
            const { execute, metricCount, metricString } = prepare(config, config.registry);
        ...
        it.each(metricDisabledTestCases(metricName, counterFactory))('should not monitor parse timing when $name', async ({ config }) => {
            const { execute, metricCount } = prepare(config, config.registry);
            const result = await execute(query);
            (0, testing_1.assertSingleExecutionValue)(result);
            expect(result.errors).toBeUndefined();
        ...
        it('should allow to use a custom name within a custom config', async () => {
            ...
            const { execute, metricCount } = prepare({
                metrics: {
                    [metricName]: (0, index_js_1.createCounter)({
                        registry,
                        counter: {
                            name: 'metric_test',
                            help: 'test',
    ...
    describe('parse', () => {
        it('should not allow empty arrays', () => {
            ...
            () => (0, index_js_1.usePrometheus)({ metrics: { graphql_envelop_phase_parse: [] } })).toThrow();
        });
        ...
        it('should allow to use custom Histogram and custom labelNames', async () => {
            ...
            const { execute, metricCount, metricString } = prepare({
                metrics: {
                    graphql_envelop_phase_parse: (0, index_js_1.createHistogram)({
                        registry,
                        histogram: {
                            name: 'test_parse',
                            help: 'HELP ME',
    ...
    describe('validate', () => {
        it('should not allow empty arrays', () => {
            expect(
            // @ts-expect-error Empty array should be disallowed
            () => (0, index_js_1.usePrometheus)({ metrics: { graphql_envelop_phase_validate: [] } })).toThrow();
        });
        ...
        it('should allow to use custom Histogram and custom labelNames', async () => {
            ...
            const { execute, metricCount, metricString } = prepare({
                metrics: {
                    graphql_envelop_phase_validate: (0, index_js_1.createHistogram)({
                        registry,
                        histogram: {
                            name: 'test_validate',
                            help: 'HELP ME',
    ...
    describe('contextBuilding', () => {
        it('should not allow empty arrays', () => {
            expect(
            // @ts-expect-error Empty array should be disallowed
            () => (0, index_js_1.usePrometheus)({ metrics: { graphql_envelop_phase_context: [] } })).toThrow();
        });
        ...
        it('should allow to use custom Histogram and custom labelNames', async () => {
            ...
            const { execute, metricCount, metricString } = prepare({
                metrics: {
                    graphql_envelop_phase_context: (0, index_js_1.createHistogram)({
                        registry,
                        histogram: {
                            name: 'test_context',
                            help: 'HELP ME',
    ...
    describe('execute', () => {
        it('should not allow empty arrays', () => {
            expect(
            // @ts-expect-error Empty array should be disallowed
            () => (0, index_js_1.usePrometheus)({ metrics: { graphql_envelop_phase_execute: [] } })).toThrow();
        });
        ...
        it('should allow to use custom Histogram and custom labelNames', async () => {
            ...
            const { execute, metricCount, metricString } = prepare({
                metrics: {
                    graphql_envelop_phase_execute: (0, index_js_1.createHistogram)({
                        registry,
                        histogram: {
                            name: 'test_execute',
                            help: 'HELP ME',
    ...
    describe('errors', () => {
        it('should not allow empty arrays', () => {
            expect(
            // @ts-expect-error Empty array should be disallowed
            () => (0, index_js_1.usePrometheus)({ metrics: { graphql_envelop_error_result: [] } })).toThrow();
        });
        const errorMetricFactory = (config) => (0, index_js_1.createCounter)({
            ...config,
            counter: {
                name: 'graphql_envelop_error_result',
                help: 'HELP ME',
        ...
        describe('parse errors', () => {
            it.each(metricEnabledTestCases('graphql_envelop_error_result', ['parse'], errorMetricFactory))('should count errors when $name', async ({ config }) => {
                const { execute, metricCount, metricString } = prepare(config, config.registry);
                const result = await execute('query {');
                (0, testing_1.assertSingleExecutionValue)(result);
                expect(result.errors?.length).toBe(1);
            ...
            it.each(metricDisabledTestCases('graphql_envelop_error_result', errorMetricFactory, [
                'validate',
                'context',
                'execute',
                'subscribe',
        ...
        describe('validate errors', () => {
            it.each(metricEnabledTestCases('graphql_envelop_error_result', ['validate'], errorMetricFactory))('should count errors when $name', async ({ config }) => {
                const { execute, metricCount, metricString } = prepare(config, config.registry);
                const result = await execute('query test($v: String!) { regularField }');
                (0, testing_1.assertSingleExecutionValue)(result);
                expect(result.errors?.length).toBe(1);
        ...
        describe('context errors', () => {
            it.each(metricEnabledTestCases('graphql_envelop_error_result', ['context'], errorMetricFactory))('should count errors when $name', async ({ config }) => {
                const { execute, metricCount, metricString } = prepare(config, config.registry, [
                    (0, core_1.useExtendContext)(() => {
                        throw new Error('error');
                    }),
        ...
        describe('execute errors', () => {
            it.each(metricEnabledTestCases('graphql_envelop_error_result', ['execute'], errorMetricFactory))('should count errors when $name', async ({ config }) => {
                const { execute, metricCount, metricString } = prepare(config, config.registry ?? new prom_client_1.Registry());
                const result = await execute('query { errorField }');
                (0, testing_1.assertSingleExecutionValue)(result);
                expect(result.errors?.length).toBe(1);
                expect(await metricCount('graphql_envelop_error_result')).toBe(1);
        ...
        it('should allow to use custom Counter and custom labelNames', async () => {
            ...
            const { execute, metricCount, metricString } = prepare({
                metrics: {
                    graphql_envelop_error_result: (0, index_js_1.createCounter)({
                        registry,
                        counter: {
                            name: 'test_error',
                            help: 'HELP ME',
                            labelNames: ['opText', 'errorMessage'],
                        },
                        fillLabelsFn: params => {
                            return {
                                opText: print(params.document),
    ...
    describe('resolvers', () => {
        ...
        it('should trace all resolvers times correctly', async () => {
            ...
            expect(await metricString('graphql_envelop_execute_resolver')).toContain('graphql_envelop_execute_resolver_count{operationName="Anonymous",operationType="query",fieldName="regularField",typeName="Query",returnType="String!"} 1');
        });
        it('should allow custom metric options', async () => {
            const registry = new prom_client_1.Registry();
            const { execute, metricCount, metricString, allMetrics } = prepare({
                metrics: {
                    graphql_envelop_execute_resolver: (0, index_js_1.createHistogram)({
                        registry,
            ...
            expect(result.errors).toBeUndefined();
            expect(await metricCount('graphql_envelop_execute_resolver', 'count')).toBe(1);
            expect(await metricString('graphql_envelop_execute_resolver')).toContain('graphql_envelop_execute_resolver_count{opText="{\\n  regularField\\n}"} 1');
        });
    ...
    describe('deprecation', () => {
        it('should not allow empty arrays', () => {
            expect(
            // @ts-expect-error Empty array should be disallowed
            () => (0, index_js_1.usePrometheus)({ metrics: { graphql_envelop_deprecated_field: [] } })).toThrow();
        });
        ...
        `, {
            ...
            expect(metric).toContain('{operationName="MutationWithDeprecatedFields",operationType="mutation",fieldName="deprecatedInput",typeName="mutationWithDeprecatedFields"}');
        });
    });
    describe('requestCount', () => {
        it('should not allow empty arrays', () => {
            expect(
            // @ts-expect-error Empty array should be disallowed
            () => (0, index_js_1.usePrometheus)({ metrics: { graphql_envelop_request: [] } })).toThrow();
        });
        ...
        it('should trace all successful requests, with multiple req', async () => {
            ...
            (0, testing_1.assertSingleExecutionValue)(result1);
            (0, testing_1.assertSingleExecutionValue)(result2);
    ...
    describe('requestSummary', () => {
        it('should not allow empty arrays', () => {
            expect(
            // @ts-expect-error Empty array should be disallowed
            () => (0, index_js_1.usePrometheus)({ metrics: { graphql_envelop_request_time_summary: [] } })).toThrow();
        });
    ...
    describe('schema', () => {
        it('should not allow empty arrays', () => {
            expect(
            // @ts-expect-error Empty array should be disallowed
            () => (0, index_js_1.usePrometheus)({ metrics: { graphql_envelop_schema_change: [] } })).toThrow();
        });
        it('should capture graphql schema changing', async () => {
            const registry = new prom_client_1.Registry();
            (0, testing_1.createTestkit)([
                (0, index_js_1.usePrometheus)({ registry, metrics: { graphql_envelop_schema_change: true } }),
                {
                    onSchemaChange: ({ replaceSchema }) => {
                        replaceSchema((0, graphql_1.buildSchema)(/* GraphQL */ `
                  type Query {
                    hello: String!
                  }
                `));
    ...
    it('should be able to register the same histogram for multiple different registries', async () => {
        const registry1 = new prom_client_1.Registry();
        const registry2 = new prom_client_1.Registry();
        const h1 = (0, utils_js_1.registerHistogram)(registry1, { name: 'h', help: 'This is a test' });
        const h2 = (0, utils_js_1.registerHistogram)(registry2, { name: 'h', help: 'This is a test' });
    ...
    it('should allow to clear the registry between initializations', async () => {
        ...
        registry.clear();
        const { execute, allMetrics } = prepare({ metrics: { graphql_envelop_phase_parse: true } }, registry);
        const result = await execute('{ regularField }');
        (0, testing_1.assertSingleExecutionValue)(result);
        expect(result.errors).toBeUndefined();
...
function metricEnabledTestCases(metricName, phases, factory) {
    return [
        {
            name: 'enabled alone',
...
function metricDisabledTestCases(metricName, factory, phases) {
    const cases = [
        {
