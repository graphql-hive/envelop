...
describe('Rate-Limiter', () => {
    ...
    describe('Programmatic', () => {
      ...
      `;
            const executeQuery = () => testkit.execute(query, {}, {
                userId: '1',
            });
            for (let i = 0; i < 5; i++) {
      ...
      `;
            for (let i = 0; i < 2; i++) {
                ...
                (0, testing_1.assertSingleExecutionValue)(resultFails);
      ...
      `;
            expect(() => testkit.execute(query, {}, { userId: '1' })).toThrow(`Config error: field 'Query.foo' has multiple matching configuration`);
        });
    });
    ...
    `;
        expect(() => testkit.execute(query, {}, { userId: '1' })).toThrow(`Config error: field 'Query.foo' has both a configuration and a directive`);
    });
