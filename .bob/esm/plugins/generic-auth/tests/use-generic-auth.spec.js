            ...
            const validateUserFn = params => {
                ...
                if (params.fieldAuthArgs) {
                    if (params.fieldAuthArgs.role !== params.user.role) {
                        return createGraphQLError(`Missing permissions for accessing field '${schemaCoordinate}'. Requires role '${params.fieldAuthArgs.role}'. Request is authenticated with role '${params.user.role}'.`, { nodes: [params.fieldNode] });
                    }
                }
                return undefined;
            };
