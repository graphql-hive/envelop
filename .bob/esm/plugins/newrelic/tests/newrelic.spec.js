...
describe('New Relic', () => {
    ...
    it('base configuration', () => {
        ...
        return helper.runInTransaction(async (tx) => {
            ...
            expect(tx.trace.root._spanContext.customAttributes.attributes.get(AttributeName.EXECUTION_OPERATION_NAME).value).toBe(`Greetings`);
            expect(tx.trace.root._spanContext.customAttributes.attributes.get(AttributeName.EXECUTION_OPERATION_TYPE).value).toBe(`query`);
            ...
            // are allowed.
            expect(helper.agent.metrics.getMetric(`Supportability/ExternalModules/${AttributeName.COMPONENT_NAME}`)).toBeTruthy();
        });
    });
    describe('includeExecuteVariables', () => {
        it('true should include all variables', () => {
            ...
            return helper.runInTransaction(async (tx) => {
                ...
                expect(tx.trace.root._spanContext.customAttributes.attributes.get(AttributeName.EXECUTION_VARIABLES).value).toBe(`{"name":"Laurin"}`);
                // Many metrics are not created until the transaction ends, if you're
                // missing metrics in your instrumentation tests, this may help.
                tx.end();
            });
        ...
        it('RegExp should include only matching variables', () => {
          ...
          `, {
                ...
                expect(tx.trace.root._spanContext.customAttributes.attributes.get(AttributeName.EXECUTION_VARIABLES).value).toBe(`{"verb":"Hi"}`);
                // Many metrics are not created until the transaction ends, if you're
                // missing metrics in your instrumentation tests, this may help.
                tx.end();
            });
    ...
    it('includeRawResult: true', () => {
        ...
        return helper.runInTransaction(async (tx) => {
            ...
            // Do some testing logic...
            await testKit.execute(`query Greetings { hello }`);
            expect(tx.trace.root._spanContext.customAttributes.attributes.get(AttributeName.EXECUTION_RESULT)
