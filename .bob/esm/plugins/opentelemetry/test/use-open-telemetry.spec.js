...
describe('useOpenTelemetry', () => {
    ...
    const useTestOpenTelemetry = (exporter, options, spanPrefix) => useOpenTelemetry({
        resolvers: false,
        result: false,
        variables: false,
        ...(options ?? {}),
  ...
  `;
    ...
    it('Should not add default resolver spans if disabled', async () => {
        const exporter = new InMemorySpanExporter();
        const testInstance = createTestkit([useTestOpenTelemetry(exporter, { resolvers: true, defaultResolvers: false })], schema);
        await testInstance.execute(/* GraphQL */ `
      query {
        obj {
          field1
    ...
    it('query should add trace_id to extensions', async () => {
        const exporter = new InMemorySpanExporter();
        const testInstance = createTestkit([useTestOpenTelemetry(exporter, { traceIdInResult: 'trace_id' })], schema);
        const result = await testInstance.execute(pingQuery);
    ...
    it('execute span should not add attribute if has inline variable', async () => {
        const exporter = new InMemorySpanExporter();
        const testInstance = createTestkit([useTestOpenTelemetry(exporter, { variables: false })], schema);
        const queryStr = /* GraphQL */ `
      query echo {
        echo(message: "hello")
      }
    ...
    it('span should not add document attribute if options false', async () => {
        const exporter = new InMemorySpanExporter();
        const testInstance = createTestkit([useTestOpenTelemetry(exporter, { document: false })], schema);
        const queryStr = /* GraphQL */ `
      query echo($message: String!) {
        echo(message: $message)
      }
