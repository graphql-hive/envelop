...
import { createSummary, registerHistogram } from '../src/utils.js';
...
const print = (ast) => graphQLPrint(ast).replace(/^\s+|\s+$/g, '');
const allMetrics = {
...
describe('Prom Metrics plugin', () => {
    ...
    function prepare(config, registry = new Registry(), plugins = []) {
        ...
        const teskit = createTestkit([
            plugin,
            useExtendContext(() => new Promise(resolve => setTimeout(resolve, 250))),
            ...plugins,
        ], schema);
        return {
    ...
    describe('parse', () => {
        it('should not allow empty arrays', () => {
            ...
            () => usePrometheus({ metrics: { graphql_envelop_phase_parse: [] } })).toThrow();
        });
    ...
    describe('validate', () => {
        it('should not allow empty arrays', () => {
            expect(
            // @ts-expect-error Empty array should be disallowed
            () => usePrometheus({ metrics: { graphql_envelop_phase_validate: [] } })).toThrow();
        });
    ...
    describe('contextBuilding', () => {
        it('should not allow empty arrays', () => {
            expect(
            // @ts-expect-error Empty array should be disallowed
            () => usePrometheus({ metrics: { graphql_envelop_phase_context: [] } })).toThrow();
        });
    ...
    describe('execute', () => {
        it('should not allow empty arrays', () => {
            expect(
            // @ts-expect-error Empty array should be disallowed
            () => usePrometheus({ metrics: { graphql_envelop_phase_execute: [] } })).toThrow();
        });
    ...
    describe('errors', () => {
        it('should not allow empty arrays', () => {
            expect(
            // @ts-expect-error Empty array should be disallowed
            () => usePrometheus({ metrics: { graphql_envelop_error_result: [] } })).toThrow();
        });
        const errorMetricFactory = (config) => createCounter({
            ...config,
            counter: {
                name: 'graphql_envelop_error_result',
                help: 'HELP ME',
        ...
        describe('execute errors', () => {
            it.each(metricEnabledTestCases('graphql_envelop_error_result', ['execute'], errorMetricFactory))('should count errors when $name', async ({ config }) => {
                const { execute, metricCount, metricString } = prepare(config, config.registry ?? new Registry());
                const result = await execute('query { errorField }');
                assertSingleExecutionValue(result);
                expect(result.errors?.length).toBe(1);
                expect(await metricCount('graphql_envelop_error_result')).toBe(1);
    ...
    describe('deprecation', () => {
        it('should not allow empty arrays', () => {
            expect(
            // @ts-expect-error Empty array should be disallowed
            () => usePrometheus({ metrics: { graphql_envelop_deprecated_field: [] } })).toThrow();
        });
    ...
    describe('requestCount', () => {
        it('should not allow empty arrays', () => {
            expect(
            // @ts-expect-error Empty array should be disallowed
            () => usePrometheus({ metrics: { graphql_envelop_request: [] } })).toThrow();
        });
    ...
    describe('requestSummary', () => {
        it('should not allow empty arrays', () => {
            expect(
            // @ts-expect-error Empty array should be disallowed
            () => usePrometheus({ metrics: { graphql_envelop_request_time_summary: [] } })).toThrow();
        });
    ...
    describe('schema', () => {
        it('should not allow empty arrays', () => {
            expect(
            // @ts-expect-error Empty array should be disallowed
            () => usePrometheus({ metrics: { graphql_envelop_schema_change: [] } })).toThrow();
        });
            ...
            createTestkit([
                usePrometheus({ registry, metrics: { graphql_envelop_schema_change: true } }),
                {
                    onSchemaChange: ({ replaceSchema }) => {
                        replaceSchema(buildSchema(/* GraphQL */ `
                  type Query {
                    hello: String!
                  }
                `));
