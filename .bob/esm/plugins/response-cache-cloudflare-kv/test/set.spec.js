...
describe('set.test.ts', () => {
    ...
    describe('set()', () => {
        ...
        test('should save the operation and entity keys in the KV store', async () => {
            await set(dataKey, dataValue, [{ typename: 'User' }, { typename: 'User', id: 1 }, { typename: 'User', id: 2 }], maxTtl, KV, keyPrefix);
        ...
        test('should function even if there are no entities', async () => {
            ...
            expect(allKeys.find(k => k.name ===
                `${buildEntityKey('User', undefined, keyPrefix)}:${operationKeyWithoutPrefix}`)).toBeUndefined();
        });
    });
});
